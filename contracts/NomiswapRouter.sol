...
contract NomiswapRouter is INomiswapRouter02 {
    ...
    function _addLiquidity(
        ...
        if (INomiswapFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            INomiswapFactory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = NomiswapLibrary.getReserves(factory, tokenA, tokenB);
    ...
    function addLiquidity(
        ...
        address pair = NomiswapLibrary.pairFor(factory, tokenA, tokenB);
    ...
    function addLiquidityETH(
        ...
        address pair = NomiswapLibrary.pairFor(factory, token, WETH);
    ...
    function removeLiquidity(
        ...
        address pair = NomiswapLibrary.pairFor(factory, tokenA, tokenB);
        INomiswapPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? NomiswapLibrary.pairFor(factory, output, path[i + 2]) : _to;
            INomiswapPair(NomiswapLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        amounts = NomiswapLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'NomiswapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, NomiswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        amounts = NomiswapLibrary.getAmountsIn(factory, amountOut, path);
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = NomiswapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'NomiswapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(NomiswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = NomiswapLibrary.sortTokens(input, output);
            INomiswapPair pair = INomiswapPair(NomiswapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = NomiswapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, pair.swapFee());
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, NomiswapLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(NomiswapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    {
        return NomiswapLibrary.getAmountOut(amountIn, reserveIn, reserveOut, swapFee);
    }
    ...
    {
        return NomiswapLibrary.getAmountIn(amountOut, reserveIn, reserveOut, swapFee);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return NomiswapLibrary.getAmountsOut(factory, amountIn, path);
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return NomiswapLibrary.getAmountsIn(factory, amountOut, path);
    }
}
